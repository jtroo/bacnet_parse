...
fn parse_unsigned(bytes: &[u8], sz: u32) -> Result<(&[u8], u32), Error> {
    let sz = sz as usize;
    if sz > 4 || sz == 0 {
        return Err(Error::InvalidValue("unsigned len value is 0 or greater than 4"))
    }
    if bytes.len() < sz {
        return Err(Error::Length("unsigned len value greater than remaining bytes"))
    }
    let val = match sz {
        1 => { bytes[0] as u32 },
        2 => { u16::from_be_bytes(*array_ref!(bytes, 0, 2)) as u32 },
        3 => { (bytes[0] << 16 | bytes[1] << 8 | bytes[2]) as u32 },
